...
uint8_t st_iterate()
{
  // Only do this if the interrupt should be enabled
  if (!(TIMSK2 & (1<<OCIE2A))) {
    return 0;
  }
// SPINDLE_ENABLE_PORT ^= 1<<SPINDLE_ENABLE_BIT; // Debug: Used to time ISR
  if (busy) { return 0; } // The busy-flag is used to avoid reentering this interrupt
  ...
  if (current_block == NULL) {
    ...
    if (current_block != NULL) {
      ...
      return 0; // Nothing to do but exit.
    }
  }
